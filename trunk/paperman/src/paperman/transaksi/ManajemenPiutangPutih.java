...
import paperman.tablemodel.PiutangPutihTableModel;

/**
 *
 * @author Nurul Chusna
 */
public class ManajemenPiutangPutih extends javax.swing.JInternalFrame {

    public static ManajemenPiutangPutih manajemenPiutangPutih;
    private List<setoranDetailPutih> listStoPthDet;
    private List<setoranPutih> listHutang;
    private setoranPutih stoPutih;
    private setoranDetailPutih stDetPutih;
    private JPopupMenu popupMenuContext;
    private Rectangle cellScopes;

    public static void inisialisasi() {
        manajemenPiutangPutih = new ManajemenPiutangPutih();
    }

    public static void destroy() {
        manajemenPiutangPutih.dispose();
        manajemenPiutangPutih = null;
    }

    public static ManajemenPiutangPutih getManajemenPiutangPutih() {
        return manajemenPiutangPutih;
    }

    /** Creates new form ManajemenPiutang */
    public ManajemenPiutangPutih() {
        initComponents();
        listHutang = Main.getTransaksiService().hutangRecordPutih(TransaksiStatus.U);
        tblPiutangData.setModel(new PiutangPutihTableModel(listHutang));
    }
...
//    }
    private void aksiCetak(setoranPutih st) {
    }

    private void aksiInfoPembayaran(setoranPutih st) {
        JasperPrint getReport = Main.getReportService().getKartuPembayaranWhiteReport(st.getDetailPutih().get(0).getKemudiPutih().getKendPutih().getNoLambung(), new DateTime(Main.getSistemService().sistemRecord().getTglKerja().getTime()).dayOfMonth().withMinimumValue().toDate(), new DateTime(Main.getSistemService().sistemRecord().getTglKerja().getTime()).dayOfMonth().withMaximumValue().toDate());
        JasperViewer.viewReport(getReport, false);
    }

    private void aksiInfoLambung(setoranPutih st) {
        JasperPrint getReport = Main.getReportService().getDataLambungForPiutangWhiteReport(st.getDetailPutih().get(0).getKemudiPutih().getKendPutih().getNoLambung(), Main.getTransaksiService().setoranPutihRecord().get(0).getTglSPO(), Main.getSistemService().sistemRecord().getTglKerja());
        JasperViewer.viewReport(getReport, false);
    }

    private void aksiCatatan(setoranPutih st) {
        new createNotesDialog().showDialogPutih(st.getDetailPutih().get(0).getKemudiPutih().getKendPutih().getNoLambung());
    }
    ...
    private void tblPiutangDataMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblPiutangDataMouseReleased
        ...
        if (OsUtils.isWindows()) {
            if (evt.isPopupTrigger()) {
                ...
                stoPutih = listHutang.get(tblPiutangData.getSelectedRow());
                ...
                JLabel titlePopup = new JLabel("Menu Lambung " + stoPutih.getDetailPutih().get(0).getKemudiPutih().getKendPutih().getNoLambung().toString());
                titlePopup.setFont(new java.awt.Font("BPG Chveulebrivi", 1, 13));
                titlePopup.setForeground(new java.awt.Color(254, 254, 254));
                pnlTitle.add(titlePopup, BorderLayout.EAST);
    ...
    private void tblPiutangDataMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblPiutangDataMouseMoved
        ...
        if (tblPiutangData.getSelectedRow() > -1) {
            stoPutih = listHutang.get(tblPiutangData.getSelectedRow());
            tblPiutangData.setToolTipText(stoPutih.getDetailPutih().get(0).getKet());
