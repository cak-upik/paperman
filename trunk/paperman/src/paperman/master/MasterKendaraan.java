...
import paperman.tablemodel.KendaraanTableModel;

/**
 *
 * @author i1440ns
 */
public class MasterKendaraan extends javax.swing.JInternalFrame implements ListSelectionListener{

    private static MasterKendaraan masterKendaraan;
    ...
    public static void inisialisasi() {
        masterKendaraan = new MasterKendaraan();
    }

    public static void destroy() {
        masterKendaraan.dispose();
        masterKendaraan = null;
    }

    public static MasterKendaraan getMasterKendaraan() {
        return masterKendaraan;
    }
    ...
    private void cekDuplicate() {
        if (validateForm()) {
            LoadFormToDatabase();
            Main.getMasterService().save(kend);
            LoadDatabaseToTable();
            housekeeping();
            enableForm(false);
    ...
    private void aksiHapus() {
        String noLambung = listKendaraan.get(tblKendaraan.getSelectedRow()).getNoLambung().toString();
        String noPol = listKendaraan.get(tblKendaraan.getSelectedRow()).getNoPolisi();
        String noMesin = listKendaraan.get(tblKendaraan.getSelectedRow()).getNoMesin();
        String noRangka = listKendaraan.get(tblKendaraan.getSelectedRow()).getNoRangka();
        String noSTNK = listKendaraan.get(tblKendaraan.getSelectedRow()).getNoSTNK();
        String jatuhTempo = listKendaraan.get(tblKendaraan.getSelectedRow()).getTglJatuhTempo().toString();
        ...
        if (konfirmasi == JOptionPane.OK_OPTION) {
            Main.getMasterService().delete(kend);
            LoadDatabaseToTable();
            housekeeping();
            kend = null;
        }
    ...
    private void LoadFormToDatabase() {
        ...
        kend.setNoLambung(Integer.valueOf(txtNoLambung.getText()));
        kend.setNoPolisi(txtNopol.getText());
        kend.setNoMesin(txtNoMesin.getText());
        kend.setNoRangka(txtNorang.getText());
        kend.setNoSTNK(txtSTNK.getText());
        kend.setTglJatuhTempo(dateJatuhTempo.getDate());
        kend.setKeterangan(txtKeterangan.getText());
    }

    private void LoadDatabaseToForm() {
        txtNoLambung.setText(kend.getNoLambung().toString());
        txtNopol.setText(kend.getNoPolisi());
        txtNoMesin.setText(kend.getNoMesin());
        txtNorang.setText(kend.getNoRangka());
        txtSTNK.setText(kend.getNoSTNK());
        dateJatuhTempo.setDate(kend.getTglJatuhTempo());
        txtKeterangan.setText(kend.getKeterangan());
    }

    private void LoadDatabaseToTable() {
        listKendaraan = Main.getMasterService().kendaraanRecord();
        tblKendaraan.setModel(new KendaraanTableModel(listKendaraan));
        int i = 0;
    ...
    public void valueChanged(ListSelectionEvent lse) {
        ...
        if (tblKendaraan.getSelectedRow() >= 0) {
            kend = listKendaraan.get(tblKendaraan.getSelectedRow());
            LoadDatabaseToForm();
            enableForm(false);
            enableSearchEngine(true);
    ...
    private void initComponents() {
        ...
        setIconifiable(true);
        setMaximizable(true);
        setTitle("Data Kendaraan - Armada Biru");
        ...
        jPanel2Layout.setVerticalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    ...
                    .addComponent(dateJatuhTempo, javax.swing.GroupLayout.DEFAULT_SIZE, 69, Short.MAX_VALUE))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jLabel8)
