...
import paperman.tablemodel.PengemudiTableModel;
...
public class MasterPengemudi extends javax.swing.JInternalFrame implements ListSelectionListener {
    ...
    private void aksiHapus() {
        ...
        String tglmasuk = listPengemudi.get(tblPengemudi.getSelectedRow()).getTglMasuk().toString();
        String kontrakBulan = listPengemudi.get(tblPengemudi.getSelectedRow()).getKontrakBulan().toString();
        String kontrakHari = listPengemudi.get(tblPengemudi.getSelectedRow()).getKontrakHari().toString();
    ...
    private void LoadFormToDatabase() {
        ...
        kemudi.setTglMasuk(tglMasuk.getDate());
        kemudi.setKontrakHari(new Integer(spinHari.getModel().getValue().toString()));
        kemudi.setKontrakBulan(new Integer(spinBulan.getModel().getValue().toString()));
    }

    private void LoadDatabaseToForm() {
        txtRefNoLambung.setText(kemudi.getKend().getNoLambung().toString());
        ...
        tglMasuk.setDate(kemudi.getTglMasuk());
        spinHari.setValue(kemudi.getKontrakHari());
        spinBulan.setValue(kemudi.getKontrakBulan());
    }
    ...
    private void txtRefNoLambungKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtRefNoLambungKeyReleased
        // TODO add your handling code here:
        listKendaraan = Main.getMasterService().findKendaraanByLambung(new Integer(txtRefNoLambung.getText()));
        kend = new kendaraan();
        if (!txtRefNoLambung.getText().isEmpty() && !listKendaraan.isEmpty()) {
            for (kendaraan k : listKendaraan) {
                kend.setId(k.getId());
                kend.setNoLambung(k.getNoLambung());
                kend.setNoPolisi(k.getNoPolisi());
                kend.setNoMesin(k.getNoMesin());
                kend.setNoRangka(k.getNoRangka());
                kend.setNoSTNK(k.getNoSTNK());
                kend.setKeterangan(k.getKeterangan());
            }
        } else {
