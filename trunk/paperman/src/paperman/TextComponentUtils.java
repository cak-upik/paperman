    ...
    public static BigDecimal parseNumberToBigDecimal(String text){
        ...
        try {
            ...
            if(Locale.US == Locale.getDefault()){
                JOptionPane.showMessageDialog(Main.getMainMenu(), "Regional setting anda menggunakan US. Pemisah ribuan adalah . (titlk) dan pemisah pecahan adalah , (koma)");
            } else if(Locale.getDefault().getCountry().equalsIgnoreCase("INDONESIA")
                    && Locale.getDefault().getLanguage().equalsIgnoreCase("ID")) {
                JOptionPane.showMessageDialog(Main.getMainMenu(), "Regional setting anda menggunakan Indonesia. Pemisah ribuan adalah , (koma) dan pemisah pecahan adalah . (titik)");
            }
        }
        return null;
    }
